...
public class ProductoDAOImpl implements ProductoDAO {   
    ...
    public List<Producto> buscar(String filtro) {
        ...
        String sql = "SELECT * FROM producto WHERE modelo LIKE ? OR marca LIKE ? OR almacenamiento LIKE ? OR color LIKE ?";
        try (Connection cn = Conexion.conectar(); PreparedStatement ps = cn.prepareStatement(sql)) {
    ...
    public List<Producto> obtenerTodos() {
        ...
        try (Connection cn = Conexion.conectar(); Statement st = cn.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            ...
            System.out.println("Error en obtenerTodos producto: " + e);
        }
        return lista;
    }
}
